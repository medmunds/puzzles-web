...
- `/puzzles`: Upstream puzzles repository (git submodule; default branch is "puzzles-web")
- `/puzzles/unreleased`: Experimental puzzles from other authors (nested git submodule, default branch is "puzzles-web")

## Build Process

### Puzzles (C/C++ to WASM)

Incremental rebuild of puzzles/* C/C++ code to WASM:

```bash
podman exec puzzles-builder /bin/bash /app/build-emcc.sh
```

If that fails with `no container with name or ID "puzzles-builder" found`,
stop and ask the human to start the puzzles-builder container.

The incremental build is recommended for quickly verifying changes in the
puzzles submodule. If you need a full build (runs in separate containers):
...
- Modify files in `/puzzles` or `/puzzles/unreleased` without understanding upstream impact (these are git submodules)
...
- `index.html.hbs` and `puzzle.html.hbs`: Templates for static page generation
...
Build process:
- `vite-extra-pages.ts`: Custom plugin for static page generation (puzzles and help)
