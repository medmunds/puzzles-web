...
export class CatalogCard extends LitElement {
  ...
  renderIcon() {
    ...
    const icon1x = `/src/assets/icons/${this.puzzleId}-64d24.png`;
    const icon2x = `/src/assets/icons/${this.puzzleId}-128d24.png`;
  ...
  render() {
    const href = this.puzzleId;
    return html`
      <sl-card @click=${this.handleCardClick}>
        <div class="card-body">
          ${this.renderIcon()}
          <div class="text">
  ...
  handleCardClick() {
    // Clicking anywhere on the card is like clicking Play
    this.shadowRoot?.querySelector<HTMLButtonElement>("#play")?.click();
  }
