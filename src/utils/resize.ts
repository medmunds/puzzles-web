 ...
 * for at least stabilityMs milliseconds (default 50). If resized callback
 * is provided, it is called on each size change while waiting.
 *
 * Useful for waiting to restore scroll position until the element
 * and its descendants have fully rendered.
 *
...
export const waitForStableSize = (
  ...
  new Promise((resolve) => {
    const { resized, stabilityMs = 50 } = options;
